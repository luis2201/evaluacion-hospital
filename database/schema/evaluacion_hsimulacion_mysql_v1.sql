...
CREATE TABLE instituciones (
    ...
    UNIQUE KEY uk_instituciones_identificacion (identificacion)
) ENGINE=InnoDB;

CREATE TABLE centros_simulacion (
    id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    ...
    created_at TIMESTAMP NULL,
    updated_at TIMESTAMP NULL,
    CONSTRAINT fk_centros_institucion
...
CREATE TABLE users (
    ...
    centro_simulacion_id BIGINT UNSIGNED NULL,
    ...
    CONSTRAINT fk_users_institucion
        FOREIGN KEY (institucion_id) REFERENCES instituciones(id)
        ON UPDATE CASCADE ON DELETE SET NULL,
    CONSTRAINT fk_users_centro
        FOREIGN KEY (centro_simulacion_id) REFERENCES centros_simulacion(id)
        ON UPDATE CASCADE ON DELETE SET NULL
) ENGINE=InnoDB;
...
CREATE TABLE evaluaciones (
    id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    centro_simulacion_id BIGINT UNSIGNED NOT NULL,
    modelo_evaluacion_id BIGINT UNSIGNED NOT NULL,
    codigo VARCHAR(50) NOT NULL,
    nombre VARCHAR(200) NOT NULL,
    descripcion TEXT NULL,
    ...
    CONSTRAINT fk_evaluaciones_centro
        FOREIGN KEY (centro_simulacion_id) REFERENCES centros_simulacion(id)
        ON UPDATE CASCADE ON DELETE RESTRICT,
...
-- Evidencia documental cargada por el responsable de un dominio.
-- Puede respaldar uno o varios descriptores del mismo dominio, sin duplicar archivos.
CREATE TABLE evidencias (
    ...
    estado ENUM('BORRADOR','ENVIADA','OBSERVADA','SUBSANADA','ACEPTADA','ARCHIVADA')
        NOT NULL DEFAULT 'BORRADOR',
    creada_por BIGINT UNSIGNED NOT NULL,
    actualizada_por BIGINT UNSIGNED NULL,
    enviada_at TIMESTAMP NULL,
    deleted_at TIMESTAMP NULL,
    created_at TIMESTAMP NULL,
    updated_at TIMESTAMP NULL,
    KEY idx_evidencias_evaluacion_dominio (evaluacion_id, dominio_id),
    KEY idx_evidencias_estado (estado),
    CONSTRAINT fk_evidencias_evaluacion
        FOREIGN KEY (evaluacion_id) REFERENCES evaluaciones(id)
        ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_evidencias_dominio
        FOREIGN KEY (dominio_id) REFERENCES dominios(id)
        ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_evidencias_creador
        FOREIGN KEY (creada_por) REFERENCES users(id)
        ON UPDATE CASCADE ON DELETE RESTRICT,
    CONSTRAINT fk_evidencias_actualizador
        FOREIGN KEY (actualizada_por) REFERENCES users(id)
        ON UPDATE CASCADE ON DELETE SET NULL
) ENGINE=InnoDB;

-- Relación N:M entre documentos y descriptores evaluables.
CREATE TABLE evidencia_descriptores (
    evidencia_id BIGINT UNSIGNED NOT NULL,
    evaluacion_descriptor_id BIGINT UNSIGNED NOT NULL,
    created_at TIMESTAMP NULL,
    PRIMARY KEY (evidencia_id, evaluacion_descriptor_id),
    KEY idx_evid_desc_descriptor (evaluacion_descriptor_id),
    CONSTRAINT fk_evid_desc_evidencia
        FOREIGN KEY (evidencia_id) REFERENCES evidencias(id)
        ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_evid_desc_eval_descriptor
        FOREIGN KEY (evaluacion_descriptor_id) REFERENCES evaluacion_descriptores(id)
        ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB;

-- Versionado: una sustitución de archivo nunca destruye la versión anterior.
CREATE TABLE evidencia_archivos (
    id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    evidencia_id BIGINT UNSIGNED NOT NULL,
    version SMALLINT UNSIGNED NOT NULL,
    disco VARCHAR(50) NOT NULL DEFAULT 'local',
    ...
    created_at TIMESTAMP NULL,
    UNIQUE KEY uk_evidencia_version (evidencia_id, version),
    UNIQUE KEY uk_evidencia_archivo_hash (evidencia_id, hash_sha256),
    CONSTRAINT fk_evidencia_archivos_evidencia
        FOREIGN KEY (evidencia_id) REFERENCES evidencias(id)
        ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT fk_evidencia_archivos_usuario
        FOREIGN KEY (cargado_por) REFERENCES users(id)
        ON UPDATE CASCADE ON DELETE RESTRICT
) ENGINE=InnoDB;

CREATE TABLE evidencia_descargas (
    id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    ...
    KEY idx_descargas_archivo (evidencia_archivo_id),
    KEY idx_descargas_usuario (user_id),
    CONSTRAINT fk_descargas_archivo
        FOREIGN KEY (evidencia_archivo_id) REFERENCES evidencia_archivos(id)
        ON UPDATE CASCADE ON DELETE CASCADE,
...
-- =============================================================
-- 7. VISTAS DE CÁLCULO
-- Regla: las evidencias sin calificar permanecen en el denominador.
...
SELECT
    ...
    COUNT(ed.id) AS total_evidencias_evaluables,
    SUM(CASE WHEN ed.calificacion IS NOT NULL THEN 1 ELSE 0 END) AS evidencias_calificadas,
    SUM(CASE WHEN ed.calificacion IS NULL THEN 1 ELSE 0 END) AS evidencias_pendientes,
...
SELECT
    ...
    SUM(rd.total_evidencias_evaluables) AS total_evidencias_evaluables,
    SUM(rd.evidencias_calificadas) AS evidencias_calificadas,
    SUM(rd.evidencias_pendientes) AS evidencias_pendientes,
    ROUND(
        SUM(rd.evidencias_calificadas) /
        NULLIF(SUM(rd.total_evidencias_evaluables), 0) * 100,
        2
    ) AS porcentaje_avance,
    ROUND(SUM(rd.aporte_ponderado_provisional), 2) AS puntaje_provisional,
    CASE
        WHEN SUM(rd.evidencias_pendientes) > 0 THEN 'INCOMPLETA'
        ELSE 'COMPLETA'
    END AS estado_calculo,
    CASE
        WHEN SUM(rd.evidencias_pendientes) > 0 THEN NULL
        WHEN SUM(rd.aporte_ponderado_provisional) < 40 THEN 'Centro Emergente'
        WHEN SUM(rd.aporte_ponderado_provisional) < 60 THEN 'Centro Consolidado'
        WHEN SUM(rd.aporte_ponderado_provisional) < 80 THEN 'Centro de alta calidad'
        ELSE 'Centro de referencia'
...
BEGIN
    ...
    IF NEW.estado = 'CERRADA' AND OLD.estado <> 'CERRADA' THEN
        ...
        IF v_total_instanciados <> v_total_modelo OR v_pendientes > 0 THEN
            SIGNAL SQLSTATE '45000'
                SET MESSAGE_TEXT = 'No se puede cerrar la evaluación: existen evidencias evaluables sin calificar o faltan ítems del modelo.';
        END IF;
    END IF;
END$$
...
VALUES
(1, 'Modelo de Evaluación de la Calidad de Centros de Simulación', '1.0',
